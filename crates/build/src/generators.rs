...
fn parse_date(date: &str) -> Date {
    ...
    Date::from_calendar_date(
        p[0].parse().unwrap(),
        Month::try_from(p[1].parse::<u8>().unwrap()).unwrap(),
        p[2].parse().unwrap(),
    )
    .unwrap()
}
...
pub fn write_feed(dist: &Path, posts: &[BlogPostSummary]) {
    ...
    for post in posts {
        ...
        let pub_date = to_utc_string(parse_date(&post.date), 0, 0, 0);
        items.push(
            [
                "    <item>".to_string(),
                format!("      <title>{}</title>", escape_xml(&post.title)),
                format!("      <link>{}</link>", escape_xml(&link)),
                format!("      <description>{}</description>", escape_xml(&post.description)),
                format!("      <pubDate>{pub_date}</pubDate>"),
                format!("      <guid isPermaLink=\"true\">{}</guid>", escape_xml(&link)),
                "    </item>".to_string(),
            ]
            .join("\n"),
        );
