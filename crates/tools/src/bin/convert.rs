...
use site::dates::{has_date_prefix, is_date_only, strip_date_prefix};
use site::frontmatter::split_frontmatter;
...
fn derive_date(file_name: &str) -> String {
    if file_name.len() >= 10 && is_date_only(&file_name[..10]) {
    ...
    std::fs::read_dir(BLOG)
        ...
        .find(|name| {
            name == &bare
                || name
                    .strip_suffix(".md")
                    .is_some_and(|stem| has_date_prefix(stem) && strip_date_prefix(stem) == slug)
        })
}
...
fn convert_file(file_name: &str) -> Result<Outcome, String> {
    ...
    let title = fm
        .title
        .clone()
        .unwrap_or_else(|| derive_title(&body, &slug));
    let description = fm
        .description
        .clone()
